...
namespace MapSync.Logic
{
    class Map:IMap
    {
        ...
        {
            int[,] map = prepareArray(sizeX, sizeY, UNKNOWN_MAP_STATE);
        ...
        public MapHolder getRequestedMapView(int topX, int topY, int sizeX, int sizeY)
        {
            ...
            else
            {
                if (parent != null)
                {
                    ...
                    currentMap = parent.getRequestedMapView(topX, topY, sizeX, sizeY);
                    initializeDirtyStatistics(currentMap);
                    return new MapHolder(currentMap);
                }
                else
